...
QUESTION 6: What are the 3 most common subscriber type for bike trips?
```sql
SELECT
  subscriber_type,
...
QUESTION 7: Which are the top 5 bike stations with the highest number of docks?
```sql
SELECT
  name,
  number_of_docks
...
QUESTION 9 : What are the total number of bike stations in Austin?
```sql
SELECT
...
QUESTION 10: How many trips start at each station, and what are the status and address of those stations?
```sql
...
QUESTION 11: How many trips are there for each subscriber type, and which types have more than 10000 trips?
```sql
SELECT
  subscriber_type,
  COUNT(*) AS trip_count
